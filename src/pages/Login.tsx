...
const Login = () => {
  ...
  return (
    <div className="min-h-screen flex relative overflow-hidden">
      <SEOHead title="Connexion" description="Connectez-vous à votre compte Dukaio pour gérer votre boutique et vos produits digitaux." canonicalPath="/login" />
      {/* Left - Form */}
      <div className="flex-1 flex items-center justify-center px-6 py-12 relative">
        {/* Ambient background on form side */}
        <motion.div
          className="absolute top-10 -left-20 h-64 w-64 rounded-full bg-primary/10 blur-[100px] pointer-events-none"
          animate={{ x: [0, 30, 0], y: [0, 20, 0], scale: [1, 1.15, 1] }}
          transition={{ duration: 11, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute bottom-10 right-0 h-56 w-56 rounded-full bg-accent/10 blur-[90px] pointer-events-none"
      ...
      <div className="hidden lg:flex flex-1 items-center justify-center bg-foreground relative overflow-hidden">
        ...
        <motion.div
          className="absolute -top-32 -right-32 h-72 w-72 rounded-full bg-primary/30 blur-[100px]"
          animate={{ x: [0, 40, -20, 0], y: [0, 30, -10, 0], scale: [1, 1.2, 0.95, 1] }}
          transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute bottom-20 left-10 h-56 w-56 rounded-full bg-primary/20 blur-[90px]"
          animate={{ x: [0, -30, 20, 0], y: [0, -20, 30, 0], scale: [1, 0.9, 1.15, 1] }}
          transition={{ duration: 14, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute top-1/3 left-1/4 h-40 w-40 rounded-full bg-accent/20 blur-[70px]"
          animate={{ x: [0, 25, -25, 0], y: [0, 25, -25, 0], scale: [1, 1.1, 1, 1] }}
        ...
        {[...Array(12)].map((_, i) => (
          <motion.div
            key={i}
            className="absolute h-1.5 w-1.5 rounded-full bg-background/40"
        ...
        >
          <motion.h2
            className="text-3xl font-extrabold text-background mb-4"
          ...
          <motion.p
            className="text-background/50 text-lg max-w-md"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
